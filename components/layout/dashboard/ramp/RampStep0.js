...
import { setStep, toggleRampTokenModal } from "@/redux/slice/rampSlice";
import Image from "next/image";

export default function RampStep0() {
  const dispatch = useDispatch();
  const selectedChain = useSelector((state) => state.ramp.selectedChain);
  const selectedToken = useSelector((state) => state.ramp.selectedToken);
  const rampTokenModal = useSelector((state) => state.ramp.rampTokenModal);
  ...
  }, [selectedToken, selectedChain, rampTokenModal]);
  ...
  return (
    <section className="flex flex-col h-full w-full gap-10 dark:text-white justify-between items-center">
      <div className="flex flex-col gap-1 w-full">
        ...
        <p className="text-sm text-gray-500">Please select a token to Buy.</p>
      </div>
      ...
      <SettingItem
        ...
        description="Select a token to buy and proceed to the next step."
