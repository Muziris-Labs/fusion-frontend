...
} from "@/redux/slice/rampSlice";
import config from "@/lib/config";
import ChainSelector from "../layout/transfer/token/ChainSelector";
import RampTokenSelector from "../layout/dashboard/ramp/RampTokenSelector";

export default function RampTokenModal() {
  const open = useSelector((state) => state.ramp.rampTokenModal);
  const dispatch = useDispatch();
  const divRef = React.useRef(null);
  const [isOverflowing, setIsOverflowing] = React.useState(false);
  const chainFilter = useSelector((state) => state.ramp.chainFilter);
